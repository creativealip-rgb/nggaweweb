    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <header className="border-b border-slate-800 px-6 py-4">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <Link href="/admin" className="text-xl font-bold hover:text-cyan-400">← Kelola Kategori</Link>
      ...
      <main className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 px-4 py-3 bg-slate-900 border border-slate-700 rounded-lg text-white focus:outline-none focus:border-cyan-500"
            placeholder="Tambah kategori baru..."
          />
          <button onClick={add} className="px-6 py-3 bg-slate-800 hover:bg-slate-700 rounded-lg transition">+ Tambah</button>
        </div>
        ...
        <p className="text-xs text-slate-500">Kategori yang dihapus dari list tidak menghapus artikel yang sudah pakai kategori tersebut.</p>
      </main>
    </div>
  );
}
